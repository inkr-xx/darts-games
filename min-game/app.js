(function ($) {
  ...
  /** Lowest score ranks first (winner). */
  ...
  /** Points added to running total (miss → MISS_VISIT_POINTS). */
  function appliedVisitPoints(buffer) {
    var raw = parsedVisitScore(buffer);
    if (raw === null) return null;
    if (raw === 0) return MISS_VISIT_POINTS;
    return raw;
  }
  ...
  function updateConfirmEnabled() {
    ...
    var ok = buf.length > 0;
    var pts = ok ? appliedVisitPoints(buf) : null;
  ...
  function confirmTurn() {
    ...
    var pts = appliedVisitPoints(buf);
